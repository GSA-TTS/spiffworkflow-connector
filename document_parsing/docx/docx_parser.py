...
class DocxParser(DocumentParser[bytes]):
    ...
    def _iter_content_controls(self, root):
    ...
    def extract_data(self) -> dict[str, str]:
        ...
        for key, sdt_content in self._iter_content_controls(root):
    ...
    def get_document_keys(self) -> list[str]:
        ...
        return list(
            dict.fromkeys(
                key for key, _ in self._iter_content_controls(root)
            )
        )
